...
public final class PotionLookerHud {
    ...
    public static void render(GuiGraphicsExtractor g, Player target, PotionLookerConfig c) {
        ...
        int height = pad * 2 + 14 + effects.size() * row;
        int x = g.guiWidth() - width - c.rightMargin;
        int y = c.topMargin;

        if (c.background) {
            int a = Math.max(0, Math.min(255, c.backgroundAlpha));
